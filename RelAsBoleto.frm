...
Begin VB.Form RelAsBoleto 
   ...
   Caption         =   "Relatório de boletos gerados"
   ClientHeight    =   1050
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7275
...
Private Sub cmdPrint_Click()
  ...
  If Trim(cpMes.Text) <> "" Then
    ...
    If chAvulsos.Value = 0 Then
      RelatoriosRPT.Carregar "", Parametros.dados, "{BOLETOS.IDSTATUS} <> 9 and {BOLETOS.TRAN} = '" & cpMes.Text & "'", "Relatório de boletos do mês " & cpMes.Text, sFormataCaminho(App.Path) & "asboletos.rpt", "{BOLETOS.COND} = " & cpCodigo.Text
    Else
      RelatoriosRPT.Carregar "", Parametros.dados, "{BOLETOS.IDSTATUS} <> 9 and left({BOLETOS.TRAN},2) = 'AV' and Month({BOLETOS.DATA}) = " & Left(cpMes.Text, 2) & " and Year({BOLETOS.DATA}) = " & Right(cpMes.Text, 4), "Relatório de boletos do mês " & cpMes.Text, sFormataCaminho(App.Path) & "asboletos.rpt", "{BOLETOS.COND} = " & cpCodigo.Text
    End If
  Else
    RelatoriosRPT.Carregar "{BOLETOS.IDSTATUS} <> 9 and {BOLETOS.VCTO};crAscendingOrder;BOLETOS|", Parametros.dados, "{BOLETOS.PAGO} = 'N'", "Relatório de boletos do mês " & cpMes.Text, sFormataCaminho(App.Path) & "asboletos.rpt", "{BOLETOS.COND} = " & cpCodigo.Text
  End If
